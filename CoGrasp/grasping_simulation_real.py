...
test_inputs = []
ur5e_handles = []
objects = ['cracker_box', 'banana', 'mug', 'phillips_screwdriver', 'hammer']
...
'hammer': gymapi.Vec3(table_pose.p.x + 0.1, table_pose.p.y + 0.3, table_dims.z),
'banana': gymapi.Vec3(table_pose.p.x, table_pose.p.y - 0.15, table_dims.z),
'cracker_box': gymapi.Vec3(table_pose.p.x - 0.2, table_pose.p.y - 0.25, table_dims.z),
'phillips_screwdriver': gymapi.Vec3(table_pose.p.x - 0.1, table_pose.p.y + 0.25, table_dims.z)}
objects_collision_mesh_files = ['/home/kaykay/isaacgym/assets/urdf/ycb/003_cracker_box/google_16k/textured_vhacd.obj',
...
simulation_result_root = '/home/kaykay/isaacgym/python/contact_graspnet/SimulationResults/Real/'
ompl_result_root = '/home/kaykay/isaacgym/python/contact_graspnet/SimulationResults/Real/'
target_positions = []
target_rotations = []
ompl_path = []
for i in range(1):
    if os.path.isfile(os.path.join(simulation_result_root, object_segments[i]) + '_ompl.npy'):
        ompl_result = np.load(os.path.join(simulation_result_root, object_segments[i]) + '_ompl.npy', allow_pickle=True)[()]
        ompl_path.append(ompl_result['pathlist'])
    else:
        ompl_path.append(None)
    object_result = np.load(os.path.join(simulation_result_root, object_segments[i]) + '.npy', allow_pickle='True')[()]
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if flag == False:
        print(f'Simulating result for {object_segments[object_idx]}')
        ...
        if dof_result:
                    ...
                    if end_state_collision:
                        ...
                        if col_free_path_found:
                            ...
                            if ompl_path[object_idx] is None:
                                simulation_result_file = "/home/kaykay/isaacgym/python/contact_graspnet/SimulationResults/Real/{}_ompl.npy".format(object_segments[object_idx])
